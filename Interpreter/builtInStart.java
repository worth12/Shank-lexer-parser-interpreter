...
public class builtInStart extends functionNode {
  ...
  public builtInStart(String name, List<variableNode> parameters, List<variableNode> constants_variables,
  ...
  public void execute(List<interpreterDataType> parameters){
    ...
    if ((parameters.get(0) instanceof arrayDataType)){
      int index = 0;
      for(int i = 0; i < ((arrayDataType)parameters.get(0)).getValue().size(); i++){
